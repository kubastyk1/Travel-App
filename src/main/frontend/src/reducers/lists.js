...
import { GET_LISTS, GET_LISTS_START, MOVE_CARD, MOVE_LIST, TOGGLE_DRAGGING } from '../actions/lists';
  ...
  switch (action.type) {
    ...
    case MOVE_CARD: {
      ...
      if (lastX === nextX) {
        ...
        // move element to new plac
        if (newLists.length-1 !== nextX) {
          newLists[nextX].cards.splice(nextY, 0, newLists[lastX].cards[lastY]);
        }
        // delete element from old place
        if (newLists.length-1 !== lastX) {
